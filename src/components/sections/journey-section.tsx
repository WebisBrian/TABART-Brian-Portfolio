...
export function JourneySection() {
  ...
  const previousStep = journeySteps[activeIndex === 0 ? journeySteps.length - 1 : activeIndex - 1];
  const nextStep = journeySteps[activeIndex === journeySteps.length - 1 ? 0 : activeIndex + 1];
          ...
          <div className="order-2 flex justify-center lg:order-1 lg:justify-start">
            <div className="relative mx-auto aspect-4/5 w-full max-w-md lg:mx-0 overflow-hidden rounded-3xl bg-muted">
              <Image
                ...
                alt={`Illustration — ${activeStep.period}`}
                fill
              ...
              {journeySteps.map((step, index) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "cursor-pointer inline-flex min-w-30 items-center justify-center gap-2 rounded-full border px-4 py-2 text-sm transition-colors",
                      isActive
            ...
            <Card className="mx-auto w-full max-w-md lg:max-w-xl">
              ...
              <CardFooter className="flex gap-3">
                <Button
                  type="button"
                  variant="outline"
                  aria-label={`Étape précédente : ${previousStep.period}`}
                ...
                <Button
                  type="button"
                  variant="outline"
                  aria-label={`Étape suivante : ${nextStep.period}`}
                  onClick={goToNext}
                >
                  Suivant
                </Button>
